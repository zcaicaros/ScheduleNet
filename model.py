...
def to_pyg(g, dev):
    ...
    feature_all = np.array(feature_all, dtype=np.float32)
    ...
    x = torch.from_numpy(np.concatenate([k_all, feature_all], axis=1))
    pyg_assigned_agent = Data(x=x, edge_index=torch.nonzero(torch.from_numpy(adj_assigned_agent)).t().contiguous())
    pyg_unassigned_agent = Data(x=x, edge_index=torch.nonzero(torch.from_numpy(adj_unassigned_agent)).t().contiguous())
    pyg_assigned_task = Data(x=x, edge_index=torch.nonzero(torch.from_numpy(adj_assigned_task)).t().contiguous())
    pyg_processable_task = Data(x=x, edge_index=torch.nonzero(torch.from_numpy(adj_processable_task)).t().contiguous())
    pyg_unprocessable_task = Data(x=x, edge_index=torch.nonzero(torch.from_numpy(adj_unprocessable_task)).t().contiguous())
    pyg_finished_task = Data(x=x, edge_index=torch.nonzero(torch.from_numpy(adj_finished_task)).t().contiguous())
    # to device
